...
from src import data, beir_utils, slurm, dist_utils, utils, contriever, finetuning_data, inbatch_frozen
...
def finetuning(opt, model, optimizer, scheduler, tokenizer, step):
    ...
    if hasattr(model, "module"):
        ...
        eval_model = model
    eval_model_query = eval_model.get_encoder('query')
    eval_model_doc = eval_model.get_encoder('doc')
    ...
    # 训练数据为(d,q_-)和(d, q_+)组成的样本对，分别由encoder_query编码doc而encoder_doc编码query
    train.eval_model(opt, eval_model_doc, eval_model_query, tokenizer, tb_logger, step)
    evaluate(opt, eval_model_doc, eval_model_query, tokenizer, tb_logger, step)
    ...
    while step < opt.total_steps:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if step >= opt.total_steps:
                break
        epoch += 1
    eval_model_doc.save_pretrained('/data/wff/project/RAG_dataset/contriever/checkpoint/my_experiments_forzen/models', safe_serialization=False)


def evaluate(opt, model_query, model_doc, tokenizer, tb_logger, step):
    ...
    model_query.eval()
    model_doc.eval()
    if hasattr(model_query, "module"):
        model_query = model_query.module
    if hasattr(model_doc, "module"):
        model_doc = model_query.module 
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            q_emb = model_query(input_ids=batch["q_tokens"], attention_mask=batch["q_mask"], normalize=opt.norm_query)
            all_emb = model_doc(input_ids=all_tokens, attention_mask=all_mask, normalize=opt.norm_doc)
...
def main():
    ...
    encoder_query, tokenizer, retriever_model_id = contriever.load_retriever(opt.model_path, opt.pooling, opt.random_init)
    encoder_doc, _, _ = contriever.load_retriever(opt.model_path, opt.pooling, opt.random_init)
    opt.retriever_model_id = retriever_model_id
    #model = inbatch_frozen.InBatchFrozen(opt, retriever, tokenizer)
    model = inbatch_frozen.InBatchFrozen(opt=opt, encoder_query=encoder_query, encoder_doc=encoder_doc, tokenizer=tokenizer)
    model = model.cuda()
