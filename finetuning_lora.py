...
from src import data, beir_utils, slurm, dist_utils, utils, contriever, finetuning_data, inbatch_lora
...
def finetuning(opt, model, optimizer, scheduler, tokenizer, step):
    ...
    # 训练数据为(d,q_-)和(d, q_+)组成的样本对，分别是encoder_query编码doc而encoder_doc编码query
    ...
    while step < opt.total_steps:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if step >= opt.total_steps:
                break
        epoch += 1
    eval_model_doc.save_pretrained('/data/wff/project/RAG_dataset/contriever/checkpoint/my_experiments_lora/models')
...
def main():
    ...
    model = inbatch_lora.InBatchLora(opt=opt, encoder_query=encoder_query, encoder_doc=encoder_doc, tokenizer=tokenizer)
    model = model.cuda()
